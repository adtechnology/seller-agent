# Seller Agent V2 — Progress

**18 open** | **1 in progress** | **20 closed** | **9 blocked** | 39 total

`[██████████░░░░░░░░░░] 51% (20/39)`

## Phase 1 — CTV/Linear Publisher Pilot

| | ID | Task | Priority | Blockers | Done |
|---|---|---|---|---|---|
| \[ ] | seller-dcd | 1A: FreeWheel Ad Server Integration | P1 | — |  |
| \[!] | seller-aq6 | &nbsp;&nbsp;↳ 1A-Phase1: FreeWheel Streaming Hub Read-Only Integration | P1 | seller-dcd |  |
| \[!] | seller-69z | &nbsp;&nbsp;↳ 1A-Phase2: FreeWheel PD/PA Deal Booking (Streaming Hub Only) | P1 | seller-aq6 |  |
| \[!] | seller-0j2 | &nbsp;&nbsp;↳ 1A-Phase3: FreeWheel PG Cross-MCP Booking + Reporting | P1 | seller-69z |  |
| \[!] | seller-jzv | 1B: FreeWheel Data Models | P1 | seller-dcd |  |
| \[!] | seller-de4 | 1C: FreeWheel CrewAI Tools | P1 | seller-dcd, seller-jzv |  |
| \[x] | seller-ide | 1D: Ad Server Abstraction Layer | P1 | — | 2026-02-26 |
| \[x] | seller-yu6 | 1E: Linear TV Specialist Agent | P1 | — | 2026-03-04 |
| \[x] | seller-v8c | 1F: Multi-turn Session Persistence | P1 | — | 2026-02-28 |
| \[x] | seller-jlt | 1G: Human-in-the-Loop Event Bus | P1 | — | 2026-02-27 |
| \[x] | seller-47z | 1H: Media Kit & Public Inventory Discovery | P1 | — | 2026-03-01 |
| \[x] | seller-aeb | 1I: Buyer Agent Authentication & Access Tiers | P1 | — | 2026-03-04 |
| \[x] | seller-5np | 1J: Agent Registry Integration & Discoverability | P1 | — | 2026-03-04 |

## Phase 2 — Negotiation & Order Lifecycle

| | ID | Task | Priority | Blockers | Done |
|---|---|---|---|---|---|
| \[x] | seller-awh | 2A: Order Workflow State Machine | P2 | — | 2026-03-09 |
| \[x] | seller-cnd | &nbsp;&nbsp;↳ 2A-1: Order Storage & API Endpoints | P2 | — | 2026-03-09 |
| \[x] | seller-5ks | &nbsp;&nbsp;↳ 2A-2: Order Audit & Reporting | P2 | — | 2026-03-09 |
| \[x] | seller-8hx | 2B: Enhanced Negotiation Engine | P1 | — | 2026-03-01 |
| \[x] | seller-ju5 | 2C: Change Request Management | P2 | — | 2026-03-09 |
| \[~] | seller-a3k | 2D: IAB Deals API v1.0 Integration | P2 | seller-dcd |  |

## Phase 3 — Platform Features & Inventory Sync

| | ID | Task | Priority | Blockers | Done |
|---|---|---|---|---|---|
| \[x] | seller-tt5 | 3A: API & Architecture Reference Documentation | P2 | — | 2026-03-10 |
| \[ ] | seller-nur | 3B: Builder Guides for Vertical Customization | P2 | — |  |
| \[x] | seller-ryn | 3C: Deployment & Operations Guide | P3 | — | 2026-03-11 |
| \[ ] | seller-48s | 3D: Scheduled Periodic Inventory Sync | P2 | — |  |
| \[ ] | seller-fro | 3E: Manual Inventory Type Mapping / Override API | P2 | — |  |
| \[ ] | seller-b4a | 3F: Rate Card Integration for Accurate Base CPMs | P2 | — |  |
| \[x] | seller-299 | 3G: Infrastructure-as-Code Deployment (CloudFormation/Terraform) | P3 | — | 2026-03-11 |
| \[ ] | seller-j2j | 3H: Incremental Sync with Change Detection | P2 | — |  |

## Phase 4 — DealJockey — Seller API

| | ID | Task | Priority | Blockers | Done |
|---|---|---|---|---|---|
| \[x] | seller-6g3 | 4A: Rename NonAgenticDSPFlow to DealRequestFlow | P2 | — | 2026-03-17 |
| \[x] | seller-a5b | 4B: Supply Chain Transparency Endpoint | P2 | — | 2026-03-17 |
| \[ ] | seller-2n2 | 4C: Template-Based Deal Creation Endpoint | P2 | — |  |
| \[x] | seller-wgd | 4D: Deal Performance Data Endpoint | P2 | — | 2026-03-17 |
| \[ ] | seller-avh | 4E: Enhanced Supply Chain with sellers.json Structure | P2 | — |  |
| \[x] | seller-gj1 | 4F: Bulk Deal Operations Endpoint | P2 | — | 2026-03-17 |
| \[!] | seller-58g | 4G: OpenDirect 3.0 Curator Support | P2 | seller-a3k |  |
| \[ ] | seller-kde | 4H: Deal Request Example | P3 | — |  |
| \[!] | seller-slf | 4I: Deal Migration Support Endpoint | P2 | seller-2n2 |  |
| \[!] | seller-om8 | 4J: Deal Deprecation API | P2 | seller-2n2 |  |
| \[ ] | seller-538 | 4K: Deal Export Formats for DSP Connectors | P3 | — |  |

## Other

| | ID | Task | Priority | Blockers | Done |
|---|---|---|---|---|---|
| \[x] | seller-9z2 | Pickup: Deal Jockey session context 2026-03-11 | P2 | — | 2026-03-23 |

---
*Last updated: 2026-03-23 16:24 UTC — auto-generated by beads*
